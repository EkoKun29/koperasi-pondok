...
<body>
    <div class="ticket">
        <p class="centered"><b><strong>KOPERASI KAMPUS {{ Auth::user()->role }}</strong></b>
            <br>
            <b><strong>NOTA PEMBELIAN HUTANG NON PRODUKSI</strong></b>
        </p>
        <p style="font-size: 1.2em; ">
            <strong>
            <br>SUPPLIER : {{  $hutangnonproduksi->nama_supplier }}
            <br>NOTA : {{ $hutangnonproduksi->no_nota }}
            <br>TANGGAL : {{ $hutangnonproduksi->created_at->format('d/m/y') }}
            <br>JENIS TRANSAKSI : HUTANG NON PRODUKSI
            <br>TANGGAL JATUH TEMPO : {{ $hutangnonproduksi->tanggal_jatuh_tempo }}
            </strong>
            <hr>
        </p>
        <table>
        ...
        <table>
            <tbody>
                <tr>
                    ...
                    <td class="description" style="text-align: right; font-weight: bold; font-size: 1.8em;">Rp {{number_format($hutangnonproduksi->total, 2, ',', '.') }}</td>
                </tr>
            </tbody>
        </table>
        <hr>
