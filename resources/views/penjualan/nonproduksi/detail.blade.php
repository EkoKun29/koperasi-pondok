...
<div class="container mx-auto px-4">
    <h1 class="text-xl font-semibold mb-4">Penjualan Detail Barang Non Produksi</h1>
    ...
    <div class="mx-4" id="modalTambahBarang">
        <a id="createPenjualanNonProduksi" style="text-decoration:none;" class="inline-block w-3 px-6 py-2 my-4 text-xs font-bold text-center text-white uppercase align-middle transition-all ease-in border-0 rounded-lg select-none shadow-soft-md bg-150 bg-x-25 leading-pro bg-gradient-to-tl from-purple-700 to-pink-500 hover:shadow-soft-2xl hover:scale-102" 
           data-bs-toggle="modal" data-bs-target="#modalTambahBarangForm{{$nonproduksi->uuid}}">Tambah Data</a>
    </div>
                    ...
                    <td class="border px-4 py-2">
                        <div class="d-flex">
                            <a href="javascript:void(0)" onclick="openEditModal({{ $dtl->id }}, '{{ $dtl->nama_barang }}', {{ $dtl->qty }}, {{ $dtl->harga }}, '{{ $dtl->keterangan }}')" class="btn btn-warning btn-sm ml-2">Edit</a>
                             <a href="{{ route('delete-penjualan-nonproduksi-detail', $dtl['id']) }}" id="btn-delete-post" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Barang {{ $dtl->nama_barang }} ??')" class="btn btn-danger btn-sm">Hapus</a>
                        </div>
                    </td>
                </tr>
                @endforeach
...
<!-- Modal Form -->
<div id="modalTambahBarangForm{{$nonproduksi->uuid}}" class="modal fade" tabindex="-1">
...
function addItem() {
    ...
    $.ajax({
        url: "{{ route('penjualan-nonproduksi-detail-create', ['uuid' => $nonproduksi->uuid]) }}",  // Update to your actual route
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
            barang: barang,
        ...
        success: function(response) {
            if (response.success) {
                    ...
                    <tr>
                        ...
                        <td class="border px-4 py-2">
                            <div class="d-flex">
                                ...
                                <a href="{{ route('delete-penjualan-nonproduksi-detail', 'id_placeholder') }}" 
                                   id="btn-delete-post" 
                ...
                resetModalForm();
                var modalElement = document.getElementById('modalTambahBarangForm{{$nonproduksi->uuid}}');
...
function updateItem() {
    ...
    $.ajax({
        url: "{{ route('penjualan-nonproduksi-update-detail', ['uuid' => $nonproduksi->uuid]) }}",
        method: 'PUT',
        data: {
            _token: '{{ csrf_token() }}',
            id: id,
