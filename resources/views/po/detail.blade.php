...
<div class="container mx-auto px-4">
    <h1 class="text-xl font-semibold mb-4">Pengajuan Detail Barang PO</h1>
    ...
    <div class="mx-4" id="modalTambahBarang">
        <a id="createPenjualanPiutang" style="text-decoration:none;" class="inline-block w-3 px-6 py-2 my-4 text-xs font-bold text-center text-white uppercase align-middle transition-all ease-in border-0 rounded-lg select-none shadow-soft-md bg-150 bg-x-25 leading-pro bg-gradient-to-tl from-purple-700 to-pink-500 hover:shadow-soft-2xl hover:scale-102" 
           data-bs-toggle="modal" data-bs-target="#modalTambahBarangForm{{$po->uuid}}">Tambah Data</a>
    </div>
        ...
        <table id="datatable-basic" class="table-auto border-collapse w-full">
            ...
            <tbody id="itemList">
                ...
                <tr>
                    ...
                    <td class="border px-4 py-2">{{ number_format($dtl->total, 2) }}</td>
                    ...
                    <div class="d-flex">
                        ...
                        <a href="{{ route('delete-pengajuan-po-detail', $dtl['id']) }}" id="btn-delete-post" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Barang {{ $dtl->nama_barang }} ??')" class="btn btn-danger btn-sm">Hapus</a>
                        </div>
                    </td>
                </tr>
                @endforeach
...
<!-- Modal Form -->
<div id="modalTambahBarangForm{{$po->uuid}}" class="modal fade" tabindex="-1">
            ...
            <form id="editPenjualanNonProduksiForm">
                ...
                <div class="mb-4">
                    ...
                    <select name="editKeterangan" id="editKeterangan" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg">
                        <option disabled selected>Pilih Keterangan</option>
                        <option value="Produksi" {{ old('keterangan', $dtl->keterangan ?? '') == 'Produksi' ? 'selected' : '' }}>Dus</option>
                        <option value="Non Produksi" {{ old('keterangan', $dtl->keterangan ?? '') == 'Non Produksi' ? 'selected' : '' }}>Pcs</option>
                    </select>
                </div>                                
                <div class="modal-footer">
...
$(document).ready(function() {
    $("#barang").select2({
        dropdownParent: $("#modalTambahBarangForm{{$po->uuid}}")
    });
...
function addItem() {
    ...
    $.ajax({
        url: "{{ route('pengajuan-po-detail-create', ['uuid' => $po->uuid]) }}",  // Update to your actual route
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
            barang: barang,
            ...
            if (response.success) {
                ...
                var newRow = `
                    <tr>
                        ...
                        <td class="border px-4 py-2">${parseFloat(response.detail.total).toFixed(2)}</td>
                        <td class="border px-4 py-2">
                            <div class="d-flex">
                                ...
                                <a href="{{ route('delete-pengajuan-po-detail', 'id_placeholder') }}" 
                                   id="btn-delete-post" 
                ...
                resetModalForm();
                var modalElement = document.getElementById('modalTambahBarangForm{{$po->uuid}}');
...
function updateItem() {
    ...
    $.ajax({
        url: "{{ route('pengajuan-po-detail-update', ['uuid' => $po->uuid]) }}",
        method: 'PUT',
        data: {
            _token: '{{ csrf_token() }}',
            id: id,
