...
<head>
    ...
    <title>Print Barang Masuk Produksi</title>
    <style>
        * {
            font-size: 8px;
...
<body>
    <div class="ticket">
        <p class="centered"><b><strong>NOTA BARANG MASUK PRODUKSI</strong></b></p>
        <p style="font-size: 1.2em; ">
            <strong>
            <br>NOTA : {{ $barangMasukProduksi->nota }}
            <br>TANGGAL MASUK: {{ \Carbon\Carbon::parse($barangMasukProduksi->created_at)->format('d-m-Y') }}
            <br>TANGGAL PEMBELIAN: {{ \Carbon\Carbon::parse($barangMasukProduksi->tanggal)->format('d-m-Y') }}
            <br>PERSONIL : {{ $barangMasukProduksi->nama_personil }}
            <br>MASUK KE- : {{ $barangMasukProduksi->masuk_ke }}
            </strong>
        </p>
