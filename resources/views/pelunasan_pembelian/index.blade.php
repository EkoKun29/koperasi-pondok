...
<div class="container mx-auto px-4">
    <h1 class="text-xl font-semibold mb-4">Pelunasan Pembelian</h1>
@if(isset($apiError))
        ...
        <table id="datatable-basic" class="table-auto border-collapse w-full">
            <thead>
                <tr class="text-left bg-gray-200">
                    ...
                    <th class="border px-4 py-2">Nota Pembelian</th>
            ...
            <tbody>
                @foreach ($pelunasan as $p)
                    <tr>
                        ...
                        <td class="border px-4 py-2">{{ $p->nama_supplier }}</td>
                        <td class="border px-4 py-2">{{ $p->pelunas }}</td>
                        <td class="border px-4 py-2">{{ $p->nota_pembelian }}</td>
                        <td class="border px-4 py-2">{{ \Carbon\Carbon::parse($p->tanggal_pembelian)->format('d-m-Y') }}</td>
                        ...
                        <div class="d-flex">
                            <a href="javascript:void(0);" data-id="{{ $p['uuid'] }}" class="btn btn-primary btn-sm ml-2 editButton">Edit</a>
                            <a href="{{ route('delete-pelunasan-pembelian', $p['uuid']) }}" id="btn-delete-post" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data Ini ??')"
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addPelunasanForm" method="POST" action="{{ route('pelunasan-pembelian.store') }}" onsubmit="const btn=document.getElementById('btn-simpan'); btn.disabled=true; btn.innerText='Menyimpan...';">
                    @csrf
                    <div class="row">
                        <!-- Left Column -->
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                <label for="add_nama_supplier" class="block text-sm font-medium text-gray-700"><b>Nama Supplier</b></label>
                                <select id="add_nama_supplier" name="nama_supplier" style="width: 100%" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg" required>
                                    <option disabled selected>Pilih Supplier</option>
                                    @foreach($dataKonsumen as $supplier)
                            ...
                            @php
                                $uniqueNota = collect($dataKonsumen)
                                    ->pluck('no_nota')  // Ambil hanya kolom no_nota
                                    ->unique()          // Hilangkan duplikat
                                    ->sort();           // Urutkan (opsional)
                            @endphp

                            <div class="mb-3">
                                <label for="add_nota_pembelian" class="block text-sm font-medium text-gray-700"><b>Nota Pembelian</b></label>
                                <select id="add_nota_pembelian" name="nota_pembelian" style="width: 100%" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg" required>
                                    <option disabled selected>Pilih Nota Pembelian</option>
                                    @foreach($uniqueNota as $nota)
                            ...
                            @php
                                $uniqueTanggal = collect($dataKonsumen)
                                    ->filter(fn($item) => $item['tanggal_valid'])
                                    ->map(function($item) {
                            ...
                            @endphp
                            <div class="mb-3">
                                <label for="add_tanggal_pembelian" class="block text-sm font-medium text-gray-700"><b>Tanggal Pembelian</b></label>
                                <select id="add_tanggal_pembelian" name="tanggal_pembelian" style="width: 100%" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg" required>
                                    <option disabled selected>Pilih Tanggal Pembelian</option>
                                   @foreach($uniqueTanggal as $tgl)
                ...
                <form id="editPelunasanForm" method="POST" onsubmit="const btn=document.getElementById('btn-simpan'); btn.disabled=true; btn.innerText='Menyimpan...';">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="mb-3">
                                <label for="edit_nama_supplier" class="block text-sm font-medium text-gray-700"><b>Nama Supplier</b></label>
                                <select id="edit_nama_supplier" name="nama_supplier" style="width: 100%" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg" required>
                                    <option disabled selected>Pilih Supplier</option>
                                    @foreach($dataKonsumen as $supplier)
                                        <option value="{{ $supplier['supplier'] }}">{{ $supplier['supplier'] }}</option>
                                    @endforeach
                            ...
                            <div class="mb-3">
                                <label for="edit_nota_pembelian" class="block text-sm font-medium text-gray-700"><b>Nota Pembelian</b></label>
                                <select id="edit_nota_pembelian" name="nota_pembelian" style="width: 100%" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg" required>
                                    <option disabled selected>Pilih Nota Pembelian</option>
                                    @foreach($uniqueNota as $nota)
                                        <option value="{{ $nota }}">{{ $nota }}</option>
                                    @endforeach
                            ...
                            @endphp
                            <div class="mb-3">
                                <label for="edit_tanggal_pembelian" class="block text-sm font-medium text-gray-700"><b>Tanggal Pembelian</b></label>
                                <select id="edit_tanggal_pembelian" name="tanggal_pembelian" style="width: 100%" class="form-input mt-1 block w-full px-3 py-2 text-lg border-2 border-gray-400 rounded-lg" required>
                                    <option disabled selected>Pilih Tanggal Pembelian</option>
                                     @foreach($uniqueTanggal as $tgl)
                                        <option value="{{ $tgl }}">{{ $tgl }}</option>
                                    @endforeach
...
<script>
    $(document).ready(function() {
        ...
        $('.editButton').on('click', function() {
            ...
            var url = "{{ route('pelunasan-pembelian.update', ':uuid') }}"; // Prepare the route
            ...
            $.ajax({
                url: '/pelunasan-pembelian/' + uuid + '/edit', // Your route for fetching pelunasan details
                method: 'GET',
                success: function(data) {
                    ...
                    $('#edit_nama_personil').val(data.pelunas).trigger('change');
                    $('#edit_nama_supplier').val(data.nama_supplier).trigger('change');
                    $('#edit_nota_pembelian').val(data.nota_pembelian).trigger('change');
                    
                    // Format tanggal_penjualan_piutang from 'YYYY-MM-DD' to 'DD-MM-YYYY'
                    var formattedDate = formatDate(data.tanggal_pembelian);
                    $('#edit_tanggal_pembelian').val(formattedDate).trigger('change');
                    ...
                    $('#editPelunasanForm').attr('action', '/pelunasan-pembelian/' + uuid);
        ...
        $('#edit_nama_personil, #edit_nama_supplier, #edit_nota_pembelian, #edit_tanggal_pembelian, #edit_bank').select2({
            dropdownParent: $('#editPelunasanModal')
        });

        // Fetch sisa_piutang when fields in add modal are selected
        $('#add_nama_supplier, #add_tanggal_pembelian, #add_nota_pembelian').on('change', function() {
            let supplier = $('#add_nama_supplier').val();
            let tanggal = $('#add_tanggal_pembelian').val();
            let no_nota = $('#add_nota_pembelian').val();

            // Ensure all fields are filled
            if (supplier && tanggal && no_nota) {
                $.ajax({
                    url: '/pelunasan-pembelian-sisa', // Replace with the correct route
                    method: 'GET',
                    data: {
                        supplier: supplier,
                        tanggal: tanggal,
        ...
        $("#add_nama_personil, #add_nama_supplier, #add_tanggal_pembelian, #add_nota_pembelian").select2({
